...
class Data_Processor():
    ...
    # plot data
    def plot_data(self, processed_data_df, user_profile, show_ac_temp=True, show_inside_temp=True, show_outside_temp=True, convert=True, time_type="week"):
...
if __name__ == "__main__":
    ...
    # # plot data
    # def plot_data(data, time_type="week"):
    #     x_axis_label = { "day": 288, "week": 7 * 288, "month": 288 * 7 * 4, "year": 288 * 52 * 7}
    #     print(data)
    #     processed_data_df = pd.DataFrame(data, columns=['Date', 'Time', 'External Temp', 'Outside Humidity', 'AC Temp', 'Internal Temp', 'AI Change', 'User Change', 'Day of Week', 'Season'])
    #     #if ac temp is -99, replace 0
    #     processed_data_df = processed_data_df.replace(-99, 0)
    #     # filter outlines >-20
    #     processed_data_df = processed_data_df[processed_data_df['AC Temp'] > -20]
    ...
    #     plt.plot(predictions.flatten()[:x_axis_label[time_type]], label="Predicat Ac Temp", color="purple")
    #     plt.plot(processed_data_df['AC Temp'][:x_axis_label[time_type]], label="AC Temp", color="red")
    #     plt.plot(processed_data_df['Internal Temp'][:x_axis_label[time_type]], label="Internal Temp", color="blue")
    #     plt.plot(processed_data_df['External Temp'][:x_axis_label[time_type]], label="External Temp", color="green")
    #     plt.legend()
    #     plt.show()
    # plot data
    def plot_data(data, time_type="week"):
        ...
        predictions, labels = LSTM_model.predict(data)
        ax.plot(predictions.flatten()[:x_axis_label[time_type]], label="Predicted AC Temp", color="purple")
        ax.plot(processed_data_df['AC Temp'][:x_axis_label[time_type]], label="AC Temp", color="red")
        ax.plot(processed_data_df['Internal Temp'][:x_axis_label[time_type]], label="Internal Temp", color="blue")
        ax.plot(processed_data_df['External Temp'][:x_axis_label[time_type]], label="External Temp", color="green")
        ax.legend()
        ...
        if time_type == "day":
            ...
            ax.xaxis.set_major_locator(mdates.DayLocator(interval=288))
            ax.xaxis.set_major_formatter(mdates.DateFormatter('%a'))
        elif time_type == "month":
            ax.xaxis.set_major_locator(mdates.DayLocator(interval=288*7))
            ax.xaxis.set_major_formatter(mdates.DateFormatter('%d'))
        elif time_type == "year":
            ax.xaxis.set_major_locator(mdates.MonthLocator(interval=int(288*7*4)))
